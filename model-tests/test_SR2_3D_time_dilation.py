...
def extract_phase_rate(samples, dt):
    ...
    Delegates to ca_propagator.phase_rate_lsq — kept here for
...
def part_B_propagation(L=48, m=0.1, kx0=0.05, n_steps=400, dt=1.0,
    ...
    # Build a single cached propagator — reused for both static + moving runs
    ...
    if show_fft_floor:
        print()
        print("FFT floor characterisation — zero-padded DFT vs lsq:")
        print("  Static channel:")
        compare_phase_methods(samples_s, dt=dt,
                              pad_factors=(1, 4, 8, 16, 32))
        print(f"  true ω (arcsin m) = {omega_s_pred:.12f}")
        print()
        print("  Moving channel:")
        compare_phase_methods(samples_m, dt=dt,
                              pad_factors=(1, 4, 8, 16, 32))
        print(f"  predicted ω (ω_k − kv_g) = {omega_m_pred:.12f}")
